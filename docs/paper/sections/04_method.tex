...
\begin{tikzpicture}[
  ...
  \node[box, fill=green!10, right=0.4cm of qual, minimum width=1.9cm] (err) {Error\\Model};
  \node[box, fill=green!10, right=0.4cm of err, minimum width=1.9cm] (art) {UMI \&\\Artefacts};
  \begin{scope}[on background layer]
    \node[worker, fill=green!3, fit=(frag)(spike)(qual)(err)(art),
...
The \texttt{learn-profile} subcommand extracts per-cycle quality distributions, per-cycle error rates, and indel rates from an existing BAM, enabling data-driven error profiles for any platform.
...
A configurable duplex conversion rate (\texttt{umi.duplex\_conversion\_rate}) controls the probability that a molecule produces both AB and BA strand families; the default is 1.0, and realistic Twist data uses ${\sim}0.90$.
...
When \texttt{umi.inline} is true, the UMI sequence and an optional spacer are prepended to R1 and R2 in both FASTQ and BAM output.
BAM records receive a leading soft-clip CIGAR op covering the prefix.
This format is compatible with downstream tools that expect inline UMIs (e.g.\ fgbio \texttt{ExtractUmisFromBam}, UMI-tools \texttt{extract}).
A per-base UMI sequencing error rate (\texttt{umi.error\_rate}) injects substitution errors into the inline UMI bases of each PCR copy, modelling the sequencing noise that UMI grouping algorithms must tolerate.
...
\subsection{Sequencing error model}

An optional layered error model (\texttt{quality.sequencing\_errors}) replaces quality-score-driven substitutions with a five-pass \texttt{ErrorOrchestrator}.
The passes are: (1) cycle-position errors, where the error rate rises with sequencing cycle; (2) k-mer context errors, applying sequence-context-dependent mismatch rates; (3) sequencing indels, inserting or deleting bases at configurable rates; (4) strand bias, modulating error rates between forward and reverse reads; and (5) phasing bursts, injecting correlated errors that mimic polymerase slippage events.
The model is configured through 14 fields in the \texttt{quality.sequencing\_errors} YAML block.
Three built-in platform presets (\texttt{illumina\_novaseq}, \texttt{pacbio\_hifi}, \texttt{nanopore\_r10}) supply default parameters calibrated to each platform's characteristic error profile.
The feature is fully opt-in: when the configuration block is absent, read generation falls back to v0.1.x quality-score-driven behaviour.
...
Built-in presets (\texttt{twist-umi-duplex}, \texttt{illumina-wgs}, \texttt{illumina-wes}, \texttt{illumina-ctdna}, \texttt{illumina-novaseq}, \texttt{pacbio-hifi}, \texttt{nanopore-r10}) set UMI, fragment, quality, and sequencing error parameters appropriate for each platform.
Explicit YAML values always take precedence over preset defaults.

\subsection{Implementation}

\varforge{} uses a pure-Rust dependency stack: noodles\autocite{noodles} for BAM, FASTA, and BGZF I/O, \texttt{flate2} for gzip compression, and direct FASTQ and VCF formatting via buffered writers.
