...
Adding threads does not improve wall time at this scale.
Single-threaded execution takes 140\,s; 12 threads takes 160\,s, a 13\% regression.
The workload is fully I/O-bound: workers complete their compute work quickly, then contend on the single writer thread draining the channel.
The coordination overhead from Rayon and crossbeam-channel dominates any compute saving.
Compute-heavy workloads (UMI simulation, high coverage) would scale better because each worker thread is occupied longer before sending to the channel.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\linewidth]{../../benchmarking/results/graphs/thread_scaling.pdf}
  \caption{Thread scaling (10\,MB, 30x, 500 variants). Performance degrades with additional threads because the workload is I/O-bound and thread coordination overhead exceeds compute savings.}
  \label{fig:threads}
\end{figure}
