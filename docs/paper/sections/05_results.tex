...
\begin{table}[t]
\centering
\caption{hg38 chr22 benchmarks (5x, 1 run each with fixed seed, 12 threads). A fixed seed produces deterministic output; repeated runs return the same result. UMI/duplex configs show higher read counts due to PCR family expansion and higher RAM from family tracking.}
\label{tab:hg38}
\small
\setlength{\tabcolsep}{3.5pt}
