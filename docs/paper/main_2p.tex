...
\begin{abstract}
\noindent
No single tool integrates de-novo read generation, somatic variant spike-in, UMI/duplex simulation, cfDNA fragmentation, library artefact injection, and platform-aware sequencing error models.
...
Each worker samples fragment lengths, positions reads, injects variants by Bernoulli sampling, applies quality scores and an optional layered sequencing error model, and tags UMI families.
...
Priority next steps: (1) end-to-end VAF validation via alignment and pileup; (2) cfDNA fragment distribution validation; (3) extending the variant caller benchmark to additional tools (VarDict, Strelka2, fgbio); (4) capturing the full joint quality correlation structure that the current layered error model does not yet reproduce.
