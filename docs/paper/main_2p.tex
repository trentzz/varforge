...
\varforge{} fills this gap in a single Rust binary driven by YAML configuration, achieving ${\sim}14{,}000$ read pairs/s at 30x on a 12-thread workstation.
...
\textbf{cfDNA.}
Fragment lengths follow a four-component Gaussian mixture with sub-nucleosomal (${\sim}143$\,bp), mono-nucleosomal (${\sim}167$\,bp), di-nucleosomal (${\sim}334$\,bp), and tri-nucleosomal (${\sim}500$\,bp) components.
...
\midrule
WGS 10\,MB, 30x  & 139.6 & 0.87 & 14,300 \\
Panel+UMI 1\,MB  & 195.1 & 1.80 &  6,800 \\
chr22, 30x WGS   & 595.3 & 4.21 &  8,537 \\
Panel BED 200x   &  85.5 & 0.56 &  7,796 \\
\bottomrule
\end{tabular}
\end{table}
...
The main limitation is that output correctness is not yet independently validated: VAF accuracy, cfDNA fragment distributions, and UMI family structure require quantitative comparison against their respective models.
...
Priority next steps: (1) chi-squared VAF accuracy test; (2) cfDNA fragment distribution validation; (3) downstream variant caller benchmarking (Mutect2 or VarDict against the truth VCF).
